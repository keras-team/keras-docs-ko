...
## 첫 예시: 완전 연결 신경망<sub>densely-connected network</sub>:

완전 연결 신경망을 구현하기에는 `Sequential` 모델이 더 적합한 선택이겠지만, 아주 간단한 예시로 설명하기 위해서 Keras 함수형 API로 구현해 보겠습니다.

- layer 인스턴스<sub>instance</sub>는 (텐서<sub>tensor</sub>에 대해) 호출 가능하고, 텐서를 반환합니다
...
## 모든 모델은 layer 인스턴스처럼 호출 가능합니다

함수형 API를 사용하면 학습된 모델을 재사용하기 편리합니다: 어느 모델이건 텐서에 대해 호출하여 layer 인스턴스처럼 사용할 수 있습니다. 모델을 호출하면 모델의 *구조*만 재사용하는 것이 아니라 가중치<sub>weights</sub>까지 재사용되는 것임을 참고하십시오.
...
예를 들어, 이를 이용하면 시퀀스 데이터<sub>sequences</sub>를 처리할 수 있는 모델을 빠르게 만들 수 있습니다. 코드 한 줄로 이미지 분류 모델을 비디오 분류 모델로 바꿀 수 있는 것입니다.
...
모델은 두 개의 손실 함수<sub>loss function</sub>에 의해서 지도됩니다. 주요 손실 함수를 모델의 초기 단계에 사용하는 것이 심층 모델에 있어 적절한 정규화<sub>regularization</sub> 메커니즘입니다.
...
보조 손실 함수를 삽입하여 모델에서 주요 손실 함수가 후기 단계에서 사용되더라도 LSTM 및 Embedding layer를 원활하게 학습 할 수 있습니다.
...
보조 손실의 가중치로 0.2를 할당하고 모델을 컴파일<sub>compile</sub>합니다.
...
입력 layer 인스턴스와 출력 layer 인스턴스에 이름을 부여 했으므로 (각 layer 인스턴스에 "name" 인자를 전달했으므로),
...
잠시 멈춰서 공유 layer 인스턴스의 출력 또는 출력 형태<sub>shape</sub>가 어떠한지 살펴봅시다.

-----

## "노드<sub>node</sub>"의 개념

입력에 대해서 layer 인스턴스를 호출하면, 새로운 텐서(layer 인스턴스의 출력)가 생성됩니다. 또한 "노드"가 layer 인스턴스에 추가되어 입력 텐서와 출력 텐서를 연결합니다. 동일한 layer 인스턴스를 여러 번 호출하면, layer 인스턴스는 0, 1, 2…와 같은 인덱스가 달린 여러 개의 노드를 소유하게 됩니다.

Keras 이전 버전에서는 `layer.get_output()`를 통해 layer 인스턴스의 출력 텐서를 얻을 수 있고, `layer.output_shape`를 통해 출력 형태를 얻을 수 있었습니다. 이러한 방식이 여전히 가능하기는 하지만 (단, `get_output()`은 `output`이라는 속성으로 대체되었습니다), layer 인스턴스가 여러 입력에 연결된 경우는 어떻게 하시겠습니까?

Layer 인스턴스가 하나의 입력에만 연결되어 있으면 `.output`은 layer 인스턴스의 단일 출력을 반환할 것입니다:
...
Layer 인스턴스가 여러 입력에 대해서 호출이 된다면 상황이 달라집니다:
```python
a = Input(shape=(280, 256))
