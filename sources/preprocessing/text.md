...
- __filters__: `str`. 입력된 텍스트로부터 걸러낼 문자를 문자열 형식으로 지정합니다. 기본값은 작은따옴표 `'`를 제외한 모든 문장부호<sub>Punctuation</sub> 및 탭과 줄바꿈 문자입니다. (기본값: ``!"#$%&()*+,-./:;<=>?@[\\]^_`{|}~\\t\\n``)
- __lower__: `bool`. 텍스트를 소문자로 변환할지의 여부를 지정합니다. 기본값은 `True`입니다.
- __split__: `str`. 문장 내 각 단어를 분리하는 단위 문자를 지정합니다. 기본값은 `' '`공백문자입니다.
...
__인자__
- __text__: `str`. 텍스트 입력.
- __filters__: `str`. 입력된 텍스트로부터 걸러낼 문자를 지정합니다. 기본값은 작은따옴표 `'`를 제외한 모든 문장부호<sub>Punctuation</sub> 및 탭과 줄바꿈 문자입니다. (기본값: ``!"#$%&()*+,-./:;<=>?@[\\]^_`{|}~\\t\\n``)
- __lower__: `bool`. 텍스트를 소문자로 변환할지의 여부를 지정합니다. 기본값은 `True`입니다.
- __split__: `str`. 문장 내 각 단어를 분리하는 단위 문자를 지정합니다. 기본값은 `' '`공백문자입니다.
