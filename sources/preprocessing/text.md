...
```
텍스트 토큰화<sub>token</sub> 도구 클래스입니다.

`Tokenizer` 클래스는 텍스트 말뭉치<sub>corpus</sub>로부터 단어를 추출하고 정수<sub>integer</sub> 인덱스를 부여하여 딕셔너리 형식의 목록을 생성합니다. 이 목록을 바탕으로 문장 입력을 각 단어의 인덱스 숫자로 이루어진 리스트로 변환하며, 반대로 인덱스 리스트를 입력할 경우 문장으로 변환합니다. 또한 문장별 단어의 등장 여부나 횟수, 비율, TF-IDF 등을 나타내는 행렬<sub>matrix</sub>을 생성할 수 있습니다. 이와 같은 처리는 `Tokenizer`아래의 개별 메소드에 의해 이루어지며, `Tokenizer`클래스는 말뭉치의 토큰화에 필요한 각종 설정값을 지정합니다. 
...
- __filters__: `str`. 입력된 텍스트로부터 걸러낼 문자를 지정합니다. 기본값은 작은따옴표 `'`를 제외한 모든 문장부호<sub>punctuation</sub> 및 탭과 줄바꿈 문자입니다.
...
- __oov_token__: `str`. 생성한 단어 인덱스`word_index`에 없는 단어를 대체하기 위한 문자열을 지정합니다 (예: `<UNK>`). 기본값은 `None`이며, 지정된 경우 목록 생성시 인덱스에 추가되어 이후 `text_to_sequence` 처리를 할 때 미등록 단어를 대체합니다.  
...
- __filters__: `str`. 입력된 텍스트로부터 걸러낼 문자를 문자열 형식으로 지정합니다. 기본값은 작은따옴표 `'`를 제외한 모든 문장부호 및 탭과 줄바꿈 문자입니다. (기본값: ``!"#$%&()*+,-./:;<=>?@[\\]^_`{|}~\\t\\n``)
- __lower__: `bool`. 텍스트를 소문자로 변환할지의 여부를 지정합니다. 기본값은 `True`입니다.
- __split__: `str`. 문장 내 각 단어를 분리하는 단위 문자를 지정합니다. 기본값은 `' '`공백문자입니다.
...
__인자__
- __text__: `str`. 텍스트 입력.
- __filters__: `str`. 입력된 텍스트로부터 걸러낼 문자를 지정합니다. 기본값은 작은따옴표 `'`를 제외한 모든 문장부호 및 탭과 줄바꿈 문자입니다. (기본값: ``!"#$%&()*+,-./:;<=>?@[\\]^_`{|}~\\t\\n``)
- __lower__: `bool`. 텍스트를 소문자로 변환할지의 여부를 지정합니다. 기본값은 `True`입니다.
- __split__: `str`. 문장 내 각 단어를 분리하는 단위 문자를 지정합니다. 기본값은 `' '`공백문자입니다.
