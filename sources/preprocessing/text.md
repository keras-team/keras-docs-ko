...
`Tokenizer` 클래스는 텍스트 말뭉치<sub>corpus</sub>로부터 단어를 추출하고 정수 인덱스를 부여하여 딕셔너리 형식의 목록을 생성합니다. 이 목록을 바탕으로 문장 입력을 각 단어의 인덱스 숫자로 이루어진 리스트로 변환하며, 반대로 인덱스 리스트를 입력할 경우 문장으로 변환합니다. 또한 문장별 단어의 등장 여부나 횟수, 비율, TF-IDF 등을 나타내는 행렬을 생성할 수 있습니다. 이와 같은 처리는 `Tokenizer` 아래의 개별 메소드에 의해 이루어지며, `Tokenizer` 클래스는 말뭉치의 토큰화에 필요한 각종 설정값을 지정합니다. 
...
', lower=True, split=' ')
```
텍스트에 일반적인 정수 대신 고정된 크기의 해시공간에서 인덱스를 부여합니다. 이때 해시 함수의 충돌로 인해 서로 다른 단어가 같은 인덱스 값을 갖는 경우가 생길 수 있습니다. 충돌 [확률](https://en.wikipedia.org/wiki/Birthday_problem#Probability_table)은 해시 공간의 차원과 개별 객체의 개수에 따라 달라집니다. `0`은 어떤 단어에도 배정되지 않는 예비 인덱스입니다.
