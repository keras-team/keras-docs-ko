...
', lower=True, split=' ')
```
텍스트에 일반적인 정수 대신 고정된 크기의 해시<sub>hash</sub>공간에서 인덱스를 부여합니다. 이때 해시 함수의 충돌로 인해 서로 다른 단어가 같은 인덱스 값을 갖는 경우가 생길 수 있습니다. 충돌 [확률](https://en.wikipedia.org/wiki/Birthday_problem#Probability_table)은 해시 공간의 차원과 개별 객체의 개수에 따라 달라집니다. `0`은 어떤 단어에도 배정되지 않는 예비 인덱스입니다.
...
- __filters__: 입력된 텍스트로부터 걸러낼 문자를 문자열 형식으로 지정합니다. 기본값은 작은따옴표 `'`를 제외한 모든 문장부호<sub>Punctuation</sub> 및 탭과 줄바꿈 문자입니다. (기본값: ``!"#$%&()*+,-./:;<=>?@[\\]^_`{|}~\\t\\n``)
- __lower__: 불리언. 텍스트를 소문자로 변환할지의 여부를 지정합니다. 기본값은 `True`입니다.
- __split__: 문장 내 각 단어를 분리하는 단위 문자를 지정합니다. 기본값은 `' '`공백문자입니다.
